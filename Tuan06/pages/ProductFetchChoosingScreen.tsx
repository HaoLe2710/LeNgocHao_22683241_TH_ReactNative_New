import { Image, StyleSheet, Text, TouchableOpacity, View } from "react-native";
import { SafeAreaView } from "react-native-safe-area-context";
import { NativeStackScreenProps } from "@react-navigation/native-stack";
import { RootProps } from "../App";
import { useEffect, useState } from "react";
import { ProductFetch } from "../models/ProductFetch";

type Props = NativeStackScreenProps<RootProps, "Home">;

export default function ProductFetchChoosingScreen({ navigation }: Props) {
  const [products, setProducts] = useState<ProductFetch[]>([]);
  const [selectedProduct, setSelectedProduct] = useState<ProductFetch | null>(
    null
  );

  useEffect(() => {
    fetch("https://68ca01fdceef5a150f6692a8.mockapi.io/vsmartyoy3")
      .then((r) => r.json())
      .then((d: ProductFetch[]) => {
        setProducts(d);
        setSelectedProduct(d[0]); // chọn mặc định màu đầu tiên
      });
  }, []);

  const changeProduct = (color: string) => {
    const found = products.find((p) => p.color === color);
    if (found) setSelectedProduct(found);
  };

  if (!selectedProduct) {
    return (
      <SafeAreaView style={styles.container}>
        <Text>Đang tải sản phẩm...</Text>
      </SafeAreaView>
    );
  }

  return (
    <SafeAreaView style={styles.container}>
      {/* Phần thông tin sản phẩm */}
      <View style={styles.productRow}>
        <Image source={{ uri: selectedProduct.image }} style={styles.image} />
        <View style={{ marginLeft: 10 }}>
          <Text style={styles.productName}>{selectedProduct.name}</Text>
          <Text>Hàng chính hãng</Text>
          <View style={{ marginTop: 10 }}>
            <Text>Màu: {selectedProduct.color}</Text>
            <Text>Cung cấp bởi Tiki Tradding</Text>
            <Text style={styles.price}>
              {selectedProduct.price.toLocaleString()} đ
            </Text>
          </View>
        </View>
      </View>

      {/* Phần chọn màu */}
      <View style={styles.colorSection}>
        <Text style={{ marginBottom: 10 }}>Chọn một màu bên dưới</Text>
        <View style={styles.colorRow}>
          <TouchableOpacity
            onPress={() => changeProduct("black")}
            style={[styles.color, styles.black]}
          />
          <TouchableOpacity
            onPress={() => changeProduct("red")}
            style={[styles.color, styles.red]}
          />
          <TouchableOpacity
            onPress={() => changeProduct("blue")}
            style={[styles.color, styles.blue]}
          />
          <TouchableOpacity
            onPress={() => changeProduct("silver")}
            style={[styles.color, styles.silver]}
          />
        </View>
        <TouchableOpacity
          onPress={() => navigation.navigate("DetailFetch", selectedProduct
          )}
          style={styles.button}
        >
          <Text style={{ color: "white", textAlign: "center" }}>Xong</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
}

const styles = StyleSheet.create({
  container: {
    flex: 1,
    padding: 20,
  },
  productRow: {
    flexDirection: "row",
    marginBottom: 20,
  },
  image: {
    width: 120,
    height: 120,
    resizeMode: "contain",
    borderRadius: 10,
  },
  productName: {
    fontSize: 18,
    fontWeight: "bold",
  },
  price: {
    marginTop: 5,
    fontSize: 16,
    fontWeight: "bold",
    color: "red",
  },
  colorSection: {
    marginTop: 20,
  },
  colorRow: {
    flexDirection: "row",
    justifyContent: "space-around",
    marginBottom: 20,
  },
  color: {
    height: 50,
    width: 50,
  },
  red: {
    backgroundColor: "#f30d0d",
  },
  blue: {
    backgroundColor: "#234896",
  },
  silver: {
    backgroundColor: "#C5F1FB",
  },
  black: {
    backgroundColor: "black",
  },
  button: {
    padding: 15,
    backgroundColor: "cyan",
    borderRadius: 10,
  },
});
